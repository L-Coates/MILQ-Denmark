...
diarrhea.vs.shannon <-ggplot(aes(x=infant.diarrhea, y=shannon_entropy, color=as.factor(infant.diarrhea)), data=metadata.v3)+geom_boxplot()+xlab("diarrhea")+ylab("Shannon entropy") +scale_x_discrete(labels=c("no\nn=287", "yes\nn=40"))+scale_color_manual(values=c(no="black", yes="#F8766D"))+theme(legend.position="none")
diarrhea.vs.observedASV <-ggplot(aes(x=infant.diarrhea, y=observed_features.boxcox.transformed, color=as.factor(infant.diarrhea)), data=metadata.v3)+geom_boxplot()+xlab("diarrhea")+ylab("observed ASVs, Box Cox transformed") +scale_x_discrete(labels=c("no\nn=287", "yes\nn=40")) +scale_color_manual(values=c(no="black", yes="#F8766D"))+theme(legend.position="none")
diarrhea.vs.evenness <-ggplot(aes(x=infant.diarrhea, y=pielou_evenness, color=as.factor(infant.diarrhea)), data=metadata.v3)+geom_boxplot()+xlab("diarrhea")+ylab("Peilou's evenness")+scale_x_discrete(labels=c("no\nn=287", "yes\nn=40"))+scale_color_manual(values=c(no="black", yes="#F8766D"))+theme(legend.position="none")
ggarrange(diarrhea.vs.shannon, diarrhea.vs.evenness,diarrhea.vs.observedASV, ncol=3, nrow=1, labels="AUTO")
ggsave("diarrhea.vs.alphadiversity.jpeg", plot=last_plot(), height=5, width=10, dpi=600)
